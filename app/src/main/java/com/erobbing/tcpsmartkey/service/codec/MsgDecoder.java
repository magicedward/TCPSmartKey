package com.erobbing.tcpsmartkey.service.codec;

import com.erobbing.tcpsmartkey.common.PackageData;
import com.erobbing.tcpsmartkey.common.PackageData.MsgHeader;
import com.erobbing.tcpsmartkey.common.TPMSConsts;
import com.erobbing.tcpsmartkey.common.req.TerminalRegisterMsg;
import com.erobbing.tcpsmartkey.common.req.TerminalRegisterMsg.TerminalRegInfo;
...
public class MsgDecoder {
    ...
    public TerminalRegisterMsg toTerminalRegisterMsg(PackageData packageData) {
        ...
        // 3. byte[4-7] 制造商ID(BYTE[5]) 5 个字节，终端制造商编码
        // byte[] tmp = new byte[5];
        body.setManufacturerId(this.parseStringFromBytes(data, 4, 4));

        // 4. byte[8-17] BYTE[10] 10 个字节，此字段暂定10个字节，是终端使用店面的ID
        body.setShopId(this.parseStringFromBytes(data, 8, 10));
        //body.setTerminalType(this.parseStringFromBytes(data, 8, 8));

        // 5. byte[18-23] BYTE[6] 6个字节,最高两位表示终端类型，00表示钥匙扣，01表示24孔钥匙箱，99表示空,
        //剩余十位数字，制造商自行定义[建议 1位表示批次，2-5 年月  6-10 序列号]，
        body.setTerminalId(this.parseStringFromBytes(data, 18, 6));
